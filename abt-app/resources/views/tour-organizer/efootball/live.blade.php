...
<body class="neon-grid-bg text-on-surface font-sans antialiased min-h-screen flex flex-col justify-between"
    ...
    <header class="bg-white/90 backdrop-blur-sm border-b border-border-subtle sticky top-0 z-30 shadow-xs">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
            <div class="flex items-center gap-3">
                <img src="{{ asset('assets/logo-abt-efootball-tur.jpg') }}" alt="eFootball Logo" class="w-9 h-9 rounded-xl object-contain border border-border-subtle p-0.5 bg-white shadow-xs shrink-0">
                <div>
            ...
            <div class="bg-white rounded-2xl border border-border-subtle p-5 sm:p-6 shadow-sm flex flex-col justify-between relative overflow-hidden"
                ...
                <div class="pt-4 mt-4 border-t border-border-subtle flex flex-wrap justify-between items-center gap-2">
                    <span class="text-xs text-secondary">
                        {{ $isFull ? 'Slot telah terkunci.' : 'Kunci tim sebelum penuh!' }}
                    </span>
                    @if(!$isFull)
                    @php
                    ...
                    <div class="flex items-center gap-2">
                        <!-- QRIS Modal Button -->
                        <button type="button" 
                                @click="openQrisModal('{{ $tournament->name }} ({{ $tournament->session_label }})', '{{ number_format($tournament->entry_fee, 0, ',', '.') }}')"
    ...
    <!-- QRIS & Rekening Pembayaran Modal Pop-up -->
    <div x-show="qrisModalOpen" x-cloak 
        ...
        <div class="bg-white rounded-2xl border border-border-subtle max-w-sm w-full p-6 shadow-2xl space-y-4 relative overflow-hidden"
             @click.outside="qrisModalOpen = false">
            
            <button type="button" @click="qrisModalOpen = false" class="absolute top-4 right-4 p-1 text-secondary hover:text-on-surface">
                <span class="material-symbols-outlined text-xl">close</span>
            </button>

            <div class="text-center">
                <div class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-primary-container/20 text-on-surface text-[10px] font-bold uppercase mb-1">
                    <span class="material-symbols-outlined text-xs">payments</span>
                    Pembayaran Registrasi
                </div>
                <h3 class="text-base font-black text-on-surface" x-text="currentSessionTitle"></h3>
                <div class="mt-1">
                    <span class="text-xs text-secondary font-medium">Biaya Pendaftaran:</span>
                    <span class="text-sm font-black font-mono text-on-surface bg-primary-container px-2 py-0.5 rounded ml-1" x-text="'Rp ' + currentFee"></span>
                </div>
            </div>

            <!-- QRIS Image Box -->
            @if(isset($qrisBase64) && $qrisBase64)
            <div class="p-2 bg-gray-50 rounded-xl border border-border-subtle text-center">
                <div class="bg-white p-2 rounded-lg border border-border-subtle inline-block shadow-xs">
                    <img src="{{ $qrisBase64 }}" alt="QRIS" class="w-48 h-auto aspect-square object-contain mx-auto rounded">
                </div>
                <p class="text-[10px] text-secondary font-semibold mt-1.5">
                    Scan via BCA Mobile, Livin, GoPay, OVO, DANA, ShopeePay, dll.
                </p>
            </div>
            @endif
            ...
            <div class="space-y-1.5 text-xs">
                ...
                <div class="flex items-center justify-between p-2 rounded-lg bg-surface border border-border-subtle">
                    <span class="font-bold text-[11px]">BCA:</span>
                    <strong class="font-mono text-xs">{{ $settings->bca_account }}</strong>
                </div>
                @endif
                @if(isset($settings->dana_account) && $settings->dana_account)
                <div class="flex items-center justify-between p-2 rounded-lg bg-surface border border-border-subtle">
                    <span class="font-bold text-[11px]">DANA:</span>
                    <strong class="font-mono text-xs">{{ $settings->dana_account }}</strong>
                </div>
                @endif
                @if(isset($settings->seabank_account) && $settings->seabank_account)
                <div class="flex items-center justify-between p-2 rounded-lg bg-surface border border-border-subtle">
                    <span class="font-bold text-[11px]">SeaBank:</span>
                    <strong class="font-mono text-xs">{{ $settings->seabank_account }}</strong>
                </div>
                @endif

                <p class="text-[10px] text-secondary text-center pt-1">
                    Semua rekening a.n. <strong class="text-on-surface font-bold">ALIEF BADRIT TAMAM</strong>
                </p>
            </div>

            <!-- Confirmation Action -->
            <div class="pt-2">
                <a :href="'https://api.whatsapp.com/send?phone=6288989504780&text=' + encodeURIComponent('Halo Admin, saya sudah transfer registrasi turnamen ' + currentSessionTitle + ' sebesar Rp ' + currentFee + '. Berikut bukti transfernya:')"
                   target="_blank"
                   class="w-full py-2.5 bg-[#25D366] text-white text-xs font-bold rounded-xl hover:brightness-95 transition flex items-center justify-center gap-1.5 shadow-xs">
