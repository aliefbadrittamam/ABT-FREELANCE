...
<div class="max-w-6xl mx-auto space-y-6" x-data="{
    activeTab: '{{ $tournament->matches->count() > 0 ? 'bracket' : 'slots' }}',
    ...
    openAdvanceModal(matchId, url, roundName, matchNum, t1Name, t1Id, t2Name, t2Id) {
        ...
        this.matchTeam1Name = t1Name || 'Tim 1 (Belum Siap)';
        this.matchTeam1Id = t1Id;
        this.matchTeam2Name = t2Name || 'Tim 2 (Belum Siap)';
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div class="flex items-center gap-2">
            ...
            <form action="{{ route('tour-organizer.efootball.start', $tournament) }}" method="POST" onsubmit="return confirm('Mulai turnamen sekarang? Status akan berubah ke Sedang Bertanding dan bagan dapat dijalankan.')" class="inline">
                @csrf
            ...
            <form action="{{ route('tour-organizer.efootball.destroy', $tournament) }}" method="POST" onsubmit="return confirm('Hapus sesi turnamen ini secara permanen?')" class="inline">
                @csrf @method('DELETE')
                <button type="submit" class="p-2 border border-red-200 dark:border-red-900/40 text-red-600 dark:text-red-400 hover:bg-red-50 rounded-lg transition" title="Hapus Sesi">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-white dark:bg-[#1e1e1e] p-4 rounded-xl border border-border-subtle dark:border-[#2a2a2a] shadow-xs text-xs">
        ...
        <div>
            <span class="text-[10px] text-secondary dark:text-gray-400 uppercase font-semibold block">Kapasitas Slot</span>
    ...
    <!-- View Mode Switcher: Daftar Slot vs Bagan Turnamen -->
    <div class="flex items-center justify-between border-b border-border-subtle dark:border-[#2a2a2a] pb-3">
        <div class="flex items-center gap-2">
            <button type="button" @click="activeTab = 'slots'"
                ...
                Daftar Slot Tim ({{ $tournament->filled_slots_count }}/{{ $tournament->max_slots }})
            </button>
            ...
            <button type="button" @click="activeTab = 'bracket'"
                ...
                Bagan Pertandingan ({{ $tournament->max_slots }} Tim)
        ...
        <a href="{{ url('/turnamen/efootball/live') }}" target="_blank" class="text-xs font-bold text-primary dark:text-primary-container hover:underline inline-flex items-center gap-1">
            Lihat Live Publik &rarr;
        </a>
    </div>

    <!-- TAB 1: DAFTAR SLOT TIM -->
                ...
                <div class="p-3 rounded-xl border transition-all flex flex-col justify-between min-h-[90px] {{ $p ? ($p->is_winner ? 'border-amber-400 bg-amber-50/50 dark:bg-amber-950/20' : 'border-border-subtle dark:border-[#333] bg-surface dark:bg-[#181818]') : 'border-dashed border-border-subtle dark:border-[#333] hover:border-primary-container bg-white dark:bg-[#1e1e1e]' }}">
                    <div class="flex items-start justify-between gap-1.5 mb-2">
                        ...
                        <div class="flex items-center gap-1">
                            ...
                            @if($tournament->status !== 'completed')
                            <form action="{{ route('tour-organizer.efootball.removeParticipant', [$tournament, $p]) }}" method="POST" onsubmit="return confirm('Keluarkan tim {{ $p->team_name }} dari slot {{ $slot }}?')" class="inline">
                                @csrf @method('DELETE')
    ...
    <!-- TAB 2: BAGAN POHON TURNAMEN (BRACKET TREE) -->
        ...
        <div class="bg-white dark:bg-[#1e1e1e] rounded-xl border border-border-subtle dark:border-[#2a2a2a] p-5 sm:p-6 shadow-xs">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6 pb-4 border-b border-border-subtle dark:border-[#2a2a2a]">
                <div>
                    ...
                    <p class="text-xs text-secondary dark:text-gray-400 mt-0.5">Klik pada kotak pertandingan untuk menentukan pemenang yang melaju ke babak berikutnya.</p>
                </div>
                ...
                <div class="flex items-center gap-2">
                    <!-- Randomize Bracket -->
                    <form action="{{ route('tour-organizer.efootball.generateBracket', $tournament) }}" method="POST" onsubmit="return confirm('Acak posisi bagan secara acak (Randomized)? Hasil pertandingan saat ini akan direset.')" class="inline">
                        @csrf
                    ...
                    <!-- Sequential Bracket -->
                    <form action="{{ route('tour-organizer.efootball.generateBracket', $tournament) }}" method="POST" onsubmit="return confirm('Buat bagan berurutan sesuai nomor slot tim?')" class="inline">
                        @csrf
            ...
            <div class="text-center py-12 space-y-3">
                ...
                <p class="text-xs text-secondary max-w-md mx-auto">
                    Pastikan minimal ada 2 tim terdaftar, lalu klik tombol <strong>Acak Bagan</strong> di atas untuk membuat pohon pertandingan.
                </p>
            </div>
            @else
            <div class="overflow-x-auto pb-6">
                <div class="flex items-stretch gap-8 min-w-[720px]">
                    ...
                    <div class="flex-1 flex flex-col justify-around space-y-6">
                        ...
                        <div class="bg-white dark:bg-[#181818] border rounded-xl p-3 shadow-xs space-y-2 transition-all {{ $m->winner_id ? 'border-emerald-300 dark:border-emerald-800' : ($m->status === 'ready' ? 'border-primary-container/80 dark:border-primary-container/40' : 'border-border-subtle dark:border-[#333] opacity-80') }}">
                            ...
                            <button type="button" 
                                    @click="openAdvanceModal({{ $m->id }}, '{{ route('tour-organizer.efootball.advanceMatch', [$tournament, $m]) }}', '{{ $m->round_name }}', {{ $m->match_number }}, '{{ addslashes($m->team1->team_name) }}', {{ $m->team1_id }}, '{{ addslashes($m->team2->team_name) }}', {{ $m->team2_id }})"
