...
<div x-data="invoiceEditForm()" class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div class="space-y-4 sm:space-y-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 p-3 bg-surface dark:bg-[#181818] rounded-lg border border-border-subtle dark:border-[#2a2a2a]">
                    <div>
                        ...
                        <div x-data="{ isCustomSubCat: @json(!empty($invoice->sub_category_custom)) }">
                ...
                <div>
                    ...
                    <textarea name="description" x-model="description" rows="3" required
                        class="w-full px-3.5 py-2.5 bg-white dark:bg-[#252525] border border-border-subtle dark:border-[#333] text-on-surface dark:text-white rounded-lg text-xs sm:text-sm focus:ring-2 focus:ring-primary outline-none resize-none"></textarea>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="total_amount" x-model="totalAmount" @input="onTotalChange()" required min="0"
                            class="w-full pl-10 sm:pl-12 pr-3.5 py-2.5 bg-white dark:bg-[#252525] border border-border-subtle dark:border-[#333] text-on-surface dark:text-white rounded-lg text-xs sm:text-sm focus:ring-2 focus:ring-primary outline-none font-bold">
                    </div>
                </div>
                ...
                <div x-show="paymentType === 'dp'" x-transition class="p-4 bg-surface dark:bg-[#181818] rounded-xl border border-border-subtle dark:border-[#2a2a2a] space-y-3">
                    ...
                    <div class="relative">
                        <span class="absolute left-3.5 top-1/2 -translate-y-1/2 text-xs sm:text-sm text-on-surface-variant dark:text-gray-400 font-bold">Rp</span>
                        <input type="number" name="dp_amount" x-model="dpAmount" @input="isCustomDp = true" min="0"
                            class="w-full pl-10 sm:pl-12 pr-3.5 py-2.5 bg-white dark:bg-[#252525] border border-border-subtle dark:border-[#333] text-on-surface dark:text-white rounded-lg text-xs sm:text-sm focus:ring-2 focus:ring-primary outline-none font-bold">
                    </div>
                ...
                <!-- Status Update -->
                <div class="p-4 bg-surface dark:bg-[#181818] rounded-xl border border-border-subtle dark:border-[#2a2a2a]">
                    <label class="block text-[11px] font-bold text-on-surface-variant dark:text-gray-300 uppercase tracking-wider mb-2">Update Status Pembayaran</label>
                    <select name="status" x-model="status" class="w-full px-3.5 py-2.5 bg-white dark:bg-[#252525] border border-border-subtle dark:border-[#333] text-on-surface dark:text-white rounded-lg text-xs sm:text-sm focus:ring-2 focus:ring-primary outline-none font-medium">
                        <option value="unpaid">Belum Bayar</option>
                        <option value="dp_paid" x-show="paymentType === 'dp'">DP Terbayar</option>
                ...
                <div class="flex items-center justify-end gap-3 pt-4 border-t border-border-subtle dark:border-[#2a2a2a]">
                    <a href="{{ route('invoices.show', $invoice) }}" class="px-5 py-2.5 bg-transparent dark:bg-[#252525] border border-border-subtle dark:border-[#333] rounded-lg text-xs sm:text-sm text-on-surface-variant dark:text-gray-300 hover:bg-surface-variant dark:hover:bg-[#333] transition font-semibold">Batal</a>
                ...
                <div class="relative z-10">
                    ...
                    <div class="flex justify-between items-start mb-5 pb-4 border-b border-border-subtle">
                        ...
                        <div class="text-right">
                            ...
                            <p class="font-bold text-on-surface text-xs font-mono tracking-wider">{{ $invoice->invoice_number }}</p>
                            <p class="text-secondary text-[11px]">{{ $invoice->created_at->format('d F Y') }}</p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4 sm:gap-6 mb-4 pb-4 border-b border-border-subtle">
                        <div>
                            <h3 class="text-[10px] sm:text-xs font-semibold text-secondary uppercase tracking-widest mb-0.5">Ditujukan Kepada</h3>
                        ...
                        <div class="flex justify-end items-start">
                            <template x-if="status === 'paid'">
                                <div class="inline-flex items-center px-3 py-1 sm:px-3.5 sm:py-1.5 rounded-full bg-status-lunas/10 text-status-lunas border border-status-lunas/20">
                                    <span class="w-1.5 h-1.5 rounded-full bg-status-lunas mr-1.5 animate-pulse"></span>
                                    <span class="text-[11px] sm:text-xs font-bold uppercase tracking-wider">Lunas</span>
                                </div>
                            </template>
                            <template x-if="status === 'dp_paid'">
                                <div class="inline-flex items-center px-3 py-1 sm:px-3.5 sm:py-1.5 rounded-full bg-status-dp/10 text-status-dp border border-status-dp/20">
                                    <span class="w-1.5 h-1.5 rounded-full bg-status-dp mr-1.5 animate-pulse"></span>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4 items-stretch">
                        ...
                        <div class="flex flex-col">
                            <p class="text-[10px] sm:text-xs font-semibold text-secondary uppercase mb-1 sm:mb-1.5">Deskripsi Pekerjaan</p>
                        ...
                        <div class="flex flex-col">
                            <p class="text-[10px] sm:text-xs font-semibold text-secondary uppercase mb-1 sm:mb-1.5">Rincian Pembayaran</p>
                            <div class="space-y-1.5 bg-surface/80 p-3 sm:p-3.5 rounded-lg border border-border-subtle flex-1 flex flex-col justify-between">
                                <div class="space-y-1">
                                    <div class="flex justify-between items-center text-xs sm:text-sm text-secondary">
...
function invoiceEditForm() {
    return {
        ...
        subCategoryCustom: @json(old('sub_category_custom', $invoice->sub_category_custom ?: '')),
        ...
        paymentType: @json(old('payment_type', $invoice->payment_type)),
        status: @json(old('status', $invoice->status)),
        totalAmount: Number(@json(old('total_amount', $invoice->total_amount))),
        dpAmount: Number(@json(old('dp_amount', $invoice->dp_amount ?: 0))),
        isCustomDp: false,

        // Hunter & Worker State
        hasWorker: @json(old('has_worker', (bool)$invoice->has_worker)),
        myRole: @json(old('my_role', $invoice->my_role ?: 'hunter')),
        paymentFlow: @json(old('payment_flow', $invoice->payment_flow ?: 'client_to_me')),
        partnerName: @json(old('partner_name', $invoice->partner_name ?: '')),
        partnerPhone: @json(old('partner_phone', $invoice->partner_phone ?: '')),
        workerPercent: Number(@json(old('worker_percentage', $invoice->worker_percentage ?: 80))),
