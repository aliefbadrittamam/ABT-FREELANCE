    ...
    <form method="GET" action="{{ route('invoices.index') }}" class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
        ...
        <div class="flex bg-surface-container dark:bg-[#1e1e1e] border border-transparent dark:border-[#2a2a2a] rounded-lg p-1 gap-1 overflow-x-auto scrollbar-none">
            <a href="{{ route('invoices.index', array_merge(request()->except('status','page'), [])) }}" 
               class="px-3 sm:px-4 py-1.5 sm:py-2 rounded-md text-xs sm:text-sm font-medium transition-colors shrink-0 {{ !request('status') ? 'bg-primary-container text-on-surface shadow-sm' : 'text-on-surface-variant dark:text-gray-400 hover:text-on-surface dark:hover:text-white' }}">
                Semua
            </a>
            <a href="{{ route('invoices.index', array_merge(request()->except('page'), ['status' => 'unpaid'])) }}" 
               class="px-3 sm:px-4 py-1.5 sm:py-2 rounded-md text-xs sm:text-sm font-medium transition-colors shrink-0 {{ request('status') === 'unpaid' ? 'bg-primary-container text-on-surface shadow-sm' : 'text-on-surface-variant dark:text-gray-400 hover:text-on-surface dark:hover:text-white' }}">
                Belum Bayar
            </a>
            <a href="{{ route('invoices.index', array_merge(request()->except('page'), ['status' => 'dp_paid'])) }}" 
               class="px-3 sm:px-4 py-1.5 sm:py-2 rounded-md text-xs sm:text-sm font-medium transition-colors shrink-0 {{ request('status') === 'dp_paid' ? 'bg-primary-container text-on-surface shadow-sm' : 'text-on-surface-variant dark:text-gray-400 hover:text-on-surface dark:hover:text-white' }}">
                DP Terbayar
            </a>
            <a href="{{ route('invoices.index', array_merge(request()->except('page'), ['status' => 'paid'])) }}" 
               class="px-3 sm:px-4 py-1.5 sm:py-2 rounded-md text-xs sm:text-sm font-medium transition-colors shrink-0 {{ request('status') === 'paid' ? 'bg-primary-container text-on-surface shadow-sm' : 'text-on-surface-variant dark:text-gray-400 hover:text-on-surface dark:hover:text-white' }}">
                Lunas
            </a>
        </div>
            ...
            <tbody class="divide-y divide-border-subtle dark:divide-[#2a2a2a] text-xs sm:text-sm">
                ...
                <tr class="hover:bg-surface-variant/30 dark:hover:bg-[#252525] transition-colors">
                    <td class="py-3.5 px-4 sm:px-6">
                        <span class="font-mono text-xs text-on-surface-variant dark:text-gray-400">{{ $invoice->invoice_number }}</span>
                    </td>
                    ...
                    <td class="py-3.5 px-4 sm:px-6 text-right">
                        <a href="{{ route('invoices.show', $invoice) }}" class="inline-flex items-center gap-1 text-xs text-primary dark:text-primary-container hover:text-on-surface transition-colors font-medium">
                            <span class="material-symbols-outlined text-lg">visibility</span>
                        </a>
                        <a href="{{ route('invoices.edit', $invoice) }}" class="inline-flex items-center gap-1 text-xs text-on-surface-variant dark:text-gray-400 hover:text-on-surface dark:hover:text-white transition-colors ml-2">
